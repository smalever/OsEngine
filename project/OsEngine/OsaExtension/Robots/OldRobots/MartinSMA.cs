 ...
 * 
 *  не актуально логика чуть изменена
 *  
 *  
...
namespace OsEngine.Robots.MyBots.MartinSMA
{
    [Bot("MartinSMA")]

    internal class MartinSMA : BotPanel
    {

        public MartinSMA(string name, StartProgram startProgram) : base(name, startProgram)
        {
            ...
            _lotEntrySize = CreateParameter("_lotEntrySize  ", 1m, 0.00001m, 100, 0.00001m);

            _takeProfitInPoints = CreateParameter("_takeProfitInPoints  ", 0.03m, 0.00001m, 1000, 0.00001m);

            _averagingToMinusStepInPoints = CreateParameter("_averagingToMinusStepInPoints  ", 0.03m, 0.00001m, 1000, 0.00001m);

            _koefAveregingInPercents = CreateParameter("_koefAveregingInPercents  ", 30m, 1, 100, 1);

            _сountAveregeSteps = CreateParameter("_сountAveregeSteps  ", 5m, 1, 50, 1);
        ...
        private void _tab_CandleFinishedEvent(List<Candle> candles)
        {
            ...
            // var a = _tab.IsConnected;
            // var b = _tab.Securiti.PriceStepCost;
            ...
            //
            if (
                _tab.PriceCenterMarketDepth < lastOpenPositionPrice - _averagingToMinusStepInPoints.ValueDecimal
                && _tab.PositionOpenLong.Count != 0
                && wereBelowSma
                && canTrade
                )
        ...
        // расчет средней цены безубытка по всем позам
        void CalculateTakeProfitAllPositions()
        {
            ...
            // нарисуем среднюю тейка всех поз 
...
*/
        ...
        /// </summary>
        private StrategyParameterDecimal _averagingToMinusStepInPoints;
